    ...
    while (1) {
        ...
        if (top->sdl_sy == V_RES && top->sdl_sx == 0) {
            ...
            if (SDL_PollEvent(&event)) {
                ...
                if (event.key.keysym.sym == SDLK_UP)    top->key_up = 1;
                if (event.key.keysym.sym == SDLK_DOWN)  top->key_down = 1;
                if (event.key.keysym.sym == SDLK_LEFT)  top->key_left = 1;
                if (event.key.keysym.sym == SDLK_RIGHT) top->key_right = 1;
            }

            if (event.type == SDL_KEYUP) {
                if (event.key.keysym.sym == SDLK_UP)    top->key_up = 0;
                if (event.key.keysym.sym == SDLK_DOWN)  top->key_down = 0;
                if (event.key.keysym.sym == SDLK_LEFT)  top->key_left = 0;
                if (event.key.keysym.sym == SDLK_RIGHT) top->key_right = 0;
            }
            }
